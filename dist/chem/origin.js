...
export class Origin {
    constructor(width, height, seed) {
        ...
        this.baseCondensationRate = 0.05;
        this.baseHydrolysisRate = 0.0018;
        this.catalystBoost = 10; // a strong nearby catalyst multiplies bonding odds up to ~11x
        this.mutationRate = 0.03; // per-base chance a templated copy mispairs
        this.energyCapacity = 140;
        this.energyFluxPerTick = 1.6; // expected new energy particles/tick (fractional, accumulated)
        this.lipidAssemblyRadius = 7;
        this.membranePermeability = 0.02; // per-tick chance a small molecule crosses a nearby membrane
        this.copyStallTimeout = 1000; // ticks a templated copy can go without completing before the complex dissociates
    ...
    templatedReplication() {
        for (const p of [...this.particles.values()]) {
            ...
            const near = this.grid
                ...
                .filter((o) => o.kind === 'nt' && o.vesicleId === p.vesicleId);
            if (near.length === 0)
                continue;
            ...
            if (mismatchRoll && chosen.code === correctBase)
                continue; // "meant" to mismatch but only correct bases nearby
            ...
            if (!this.rng.bool(Math.min(0.9, 0.04 * boost)))
                continue;
            this.removeParticle(energyNearby.id);
    ...
    getStats() {
        ...
        for (const v of this.vesicles.values())
            totalReplicationEvents += v.replicationEvents;
        return {
            tick: this.tick,
