...
import { DIVISION_LIPID_COUNT, isBootstrapEligible, MIN_VESICLE_LIPIDS, OVERSIZE_DIVISION_MULTIPLIER, radiusForLipidCount, } from './vesicle.js';
...
export class Origin {
    constructor(width, height, seed, ventEnabled = true) {
        ...
        this.ventCurrentStrength = 0.015;
        this.ventRadialStrength = 0.015;
        ...
        this.complementaryFusionChance = 0.9;
        ...
        // instant "swimming."
        this.vesicleChemotaxisBiasStrength = 0.002;
        ...
        this.divisionSiblingCooldownTicks = 50;
        ...
        // clears the bar.
        this.catalyticFoldProtection = 0.4;
    ...
    static seedPrimordialSoup(width, height, seed, opts = {}) {
        ...
        if (o.vent)
            o.vent = { x: seedPatch.x + seedPatch.w * 0.15, y: seedPatch.y + seedPatch.h * 0.5 };
        for (const code of AMINO_ACID_CODES)
            for (let i = 0; i < 18 * SOUP_DENSITY_MULTIPLIER; i++)
                o.spawnAminoAcid(code, o.randomPosInRect(seedPatch));
        for (const code of NUCLEOTIDE_CODES)
            for (let i = 0; i < 70 * SOUP_DENSITY_MULTIPLIER; i++)
                o.spawnNucleotide(code, o.randomPosInRect(seedPatch));
        for (let i = 0; i < 240 * SOUP_DENSITY_MULTIPLIER; i++)
            o.spawnLipid(o.randomPosInRect(seedPatch));
        for (let i = 0; i < 60 * SOUP_DENSITY_MULTIPLIER; i++)
            o.spawnEnergy(o.randomPosInRect(seedPatch));
        return o;
    }
    // --- spawning ----------------------------------------------------------
    randomPos() {
     ...
     * velocity. */
    spawnAminoAcid(code, at, jet = false) {
        const p = {
            id: this.nextId++,
            kind: 'aa',
            code,
    ...
    spawnNucleotide(code, at, jet = false) {
        const p = {
            id: this.nextId++,
            kind: 'nt',
            code,
    ...
    spawnManualPeptide(at, length, forceCatalyst) {
        ...
        for (let i = 0; i < attempts; i++) {
            ...
            if (!forceCatalyst || fold.isCatalyst)
                break;
        }
        const p = {
            id: this.nextId++,
    ...
    spawnManualRna(at, length, forceRibozyme) {
        const len = clamp(Math.round(length), 2, MAX_POLYMER_LENGTH);
        let sequence = [];
        let fold = null;
        ...
        for (let i = 0; i < attempts; i++) {
            ...
            if (!forceRibozyme || fold.isRibozyme)
                break;
        }
        const p = {
            id: this.nextId++,
    ...
    moveParticles() {
        for (const p of this.particles.values()) {
            ...
            const isFreeAbiotic = p.vesicleId === null && (p.kind === 'aa' || p.kind === 'nt' || p.kind === 'lipid' || p.kind === 'energy');
            if (outOfBounds && isFreeAbiotic) {
                ...
                if (p.kind !== 'energy')
                    this.ventInjected[p.kind] = Math.max(0, this.ventInjected[p.kind] - 1);
                continue;
            }
    ...
    hydrolyze() {
        for (const p of [...this.particles.values()]) {
            if (p.kind === 'peptide') {
                ...
                if (this.rng.bool(this.baseHydrolysisRate * resistance * foldedProtection))
        ...
        for (const v of [...this.vesicles.values()]) {
            ...
            if (nearbyFree.length > 0) {
                ...
                const newlyEnclosed = this.grid
                    .queryRadius(v.x, v.y, v.radius + 3)
                    .filter((p) => (p.kind === 'peptide' || p.kind === 'rna') && p.vesicleId === null);
                for (const p of newlyEnclosed) {
                    if (Math.hypot(p.x - v.x, p.y - v.y) > v.radius)
                        continue;
                    p.vesicleId = v.id;
                    v.memberIds.add(p.id);
                }
    ...
    vesicleChemotaxis() {
        ...
        for (const v of list) {
            ...
            if (!catalystOnly && !replicatorOnly)
                continue;
            let nearest = null;
    ...
    fuseVesicles() {
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            for (let j = i + 1; j < list.length; j++) {
                ...
                if ((a.siblingId === b.id || b.siblingId === a.id) &&
