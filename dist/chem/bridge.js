...
import { CORE_GENE_COUNT, decodeProteinGene, GENE_LENGTH, LOCUS, PROTEIN_GENE_LENGTH } from '../sim/genes.js';
...
function flattenRnaSymbols(candidate) {
    const symbols = [];
...
export function translateBootstrapCandidate(candidate) {
    ...
    for (const gene of proteins.genes)
        scan(gene);
    const isViable = () => hasEnergyCapture || (hasPredation && hasMotor);
