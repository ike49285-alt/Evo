...
const peptideFoldCache = new Map();
const rnaFoldCache = new Map();
...
export function foldPeptide(sequence) {
    ...
    const occupied = new Map(); // "x,y" -> chain index
    const positions = [[0, 0]];
    occupied.set('0,0', 0);
    let contacts = 0;
    let boxedIn = false;
    for (let i = 1; i < sequence.length; i++) {
        ...
        for (const [dx, dy] of DIRS) {
            ...
            if (proline && prevDir && dx === prevDir[0] && dy === prevDir[1])
                continue;
            const nx = px + dx;
            const ny = py + dy;
            const nkey = `${nx},${ny}`;
            ...
            if (isH) {
                for (const [ex, ey] of DIRS) {
                    ...
                    if (ni !== undefined && ni !== i - 1 && isHydrophobic(sequence[ni]))
                        gain++;
                }
            }
    ...
    for (let i = 0; i < positions.length; i++) {
        if (!AMINO_ACIDS[sequence[i]].formsDisulfide)
            continue;
        const [x, y] = positions[i];
        for (const [dx, dy] of DIRS) {
            const ni = occupied.get(`${x + dx},${y + dy}`);
            if (ni !== undefined && ni > i && AMINO_ACIDS[sequence[ni]].formsDisulfide)
                disulfides++;
        }
    }
...
const MAX_LOOP = 8;
export function foldRna(sequence) {
    const key = sequence.join('');
