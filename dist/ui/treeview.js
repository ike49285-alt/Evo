...
export function defaultTreeCamera() {
    return { cx: 0.5, cy: 0.5, zoom: 1 };
}
export function panTreeCamera(cam, dxCss, dyCss, w, h) {
...
export function zoomTreeCameraAt(cam, sxCss, syCss, factor, w, h) {
...
export function drawTree(canvas, nodes, opts) {
    ...
    for (;;) {
        if (frontier.length >= leafBudget)
            break;
        let bestIdx = -1;
        let bestWeight = -1;
        for (let i = 0; i < frontier.length; i++) {
            if (childrenOf(frontier[i]).length === 0)
                continue;
        ...
        if (bestIdx === -1)
            break; // every frontier node is a real leaf already
    ...
    const drawnChildrenOf = (id) => frontierSet.has(id) ? [] : childrenOf(id).filter((c) => drawn.has(c));
    ...
    const summarise = (id) => {
        ...
        if (self && childrenOf(id).length > 0) {
            ...
            while (stack.length) {
                ...
                for (const kid of childrenOf(curId))
                    stack.push(kid);
            }
            if (bestId !== -1) {
                repId = bestId;
    ...
    const drawnTick = (id) => frontierSet.has(id) && childrenOf(id).length > 0
        ? summarise(id).tick
    ...
    const tipWeight = (id) => frontierSet.has(id) && childrenOf(id).length > 0 ? Math.max(1, nodes.get(id)?.liveCount ?? 1) : 1;
    ...
    const originX = (id) => view(xOf(nodes.get(id)?.birthTick ?? 0), 0).x;
    const markerX = (id) => view(xOf(drawnTick(id)), 0).x;
    const rowY = (id) => view(0, yOf(slot.get(id) ?? 0)).y;
    for (const id of drawn) {
        if (!nodes.has(id))
            continue;
    ...
    const posOf = (id) => positions.get(selectableId.get(id) ?? id);
    ...
    for (const id of drawn) {
        ...
        if (!node || node.parentId === null || !drawn.has(node.parentId))
            continue;
