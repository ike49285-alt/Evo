...
export function drawSpeciesTree(canvas, nodes, opts) {
    ...
    const liveWeight = (id) => {
        ...
        for (const kid of childrenOf(id))
            total += liveWeight(kid);
        liveWeightCache.set(id, total);
        return total;
    };
    const showsAlive = (id) => frontierSet.has(id) && childrenOf(id).length > 0 ? liveWeight(id) > 0 : !(nodes.get(id)?.isExtinct ?? true);
    // --- vertical layout ----------------------------------------------------
    let nextSlot = 0;
    const slot = new Map();
    const visitStack = [];
    ...
    const startX = (id) => view(xOf(nodes.get(id)?.createdTick ?? 0), 0).x;
    const markerX = (id) => view(xOf(endTick(id)), 0).x;
