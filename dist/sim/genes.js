 ...
 */
import { CODON_TABLE, NUCLEOTIDE_CODES } from '../chem/elements.js';
import { CATALYSIS_CLASSES, foldPeptide } from '../chem/polymer.js';
import { TRAIT_LIMITS } from './types.js';
 ...
 * the whole lineage. */
export function cloneGeneSequence(seq) {
...
export function validateGeneSequence(seq) {
    ...
    for (let i = 0; i < seq.genes.length; i++) {
        ...
        for (let j = 0; j < gene.length; j++) {
            ...
            if (!NUCLEOTIDE_CODES.includes(gene[j])) {
...
function decodeUnitFromSymbols(symbols) {
    let acc = 0;
    for (const s of symbols)
        acc += NUCLEOTIDE_CODES.indexOf(s);
    const max = symbols.length * (NUCLEOTIDE_CODES.length - 1);
    return max > 0 ? acc / max : 0;
}
export function decodeUnit(gene) {
    return decodeUnitFromSymbols(gene);
}
function lerp(unit, min, max) {
...
export function translateProteinGene(gene) {
    const aminoAcids = [];
    for (let i = 0; i + 3 <= gene.length; i += 3) {
        const codon = (gene[i] + gene[i + 1] + gene[i + 2]);
        const residue = CODON_TABLE.get(codon);
        if (residue === undefined || residue === 'STOP')
            break;
...
export function decodeProteinGene(gene) {
...
export function decodeCoreTraits(seq) {
    return {
...
export function decodeProteins(seq) {
    return seq.genes.slice(CORE_GENE_COUNT).map(decodeProteinGene);
}
// --- construction ----------------------------------------------------------
function randomGene(rng, length) {
    const g = [];
    for (let i = 0; i < length; i++)
        g.push(rng.pick(NUCLEOTIDE_CODES));
    return g;
}
export function randomGeneSequence(rng, proteinGeneCount) {
    const count = Math.min(TRAIT_LIMITS.maxProteins, Math.max(0, proteinGeneCount));
    const genes = [];
    for (let i = 0; i < CORE_GENE_COUNT; i++)
        genes.push(randomGene(rng, GENE_LENGTH));
    for (let i = 0; i < count; i++)
        genes.push(randomGene(rng, PROTEIN_GENE_LENGTH));
    return { genes };
}
// --- mutation ----------------------------------------------------------
...
export function mutateGeneSequence(parent, rng, pointMutationRateMultiplier = 1) {
    ...
    if (proteinGenes.length > TRAIT_LIMITS.maxProteins)
        proteinGenes = proteinGenes.slice(0, TRAIT_LIMITS.maxProteins);
...
export function crossoverGeneSequence(a, b, rng) {
    const coreGenes = [];
    for (let i = 0; i < CORE_GENE_COUNT; i++)
        coreGenes.push(rng.bool(0.5) ? a.genes[i] : b.genes[i]);
...
function distanceFromParts(aGenes, aTally, bGenes, bTally) {
    let coreDist = 0;
    for (let i = 0; i < CORE_GENE_COUNT; i++)
        coreDist += Math.abs(decodeUnit(aGenes[i]) - decodeUnit(bGenes[i]));
...
function catalysisClassHistogram(seq) {
    const hist = {};
