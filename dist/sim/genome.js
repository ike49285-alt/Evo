...
import { CORE_GENE_COUNT, crossoverGeneSequence, decodeCoreTraits, decodeOrganelles, GENE_LENGTH, LOCUS, mutateGeneSequence, randomGeneSequence, } from './genes.js';
...
function kindToUnit(kind) {
    ...
    for (const bucket of ORGANELLE_KIND_BUCKETS) {
        if (bucket.kind === kind)
            return (acc + bucket.weight / 2) / ORGANELLE_WEIGHT_TOTAL; // bucket midpoint
        acc += bucket.weight;
    }
    return 0;
}
export function encodeOrganelleGene(organelle) {
...
export function encodeGeneSequence(traits, organelles) {
    const core = new Array(CORE_GENE_COUNT);
    ...
    core[LOCUS.senseRadius] = encodeUnit((traits.senseRadius - TRAIT_LIMITS.senseRadius.min) / (TRAIT_LIMITS.senseRadius.max - TRAIT_LIMITS.senseRadius.min), GENE_LENGTH);
...
export function buildGenome(traits, organelles, brain) {
