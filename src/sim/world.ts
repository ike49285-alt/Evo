...
export class World {
  ...
  readonly maxPlantFood = 700;
  readonly plantSpawnRate = 2.5; // pellets per tick
  ...
  seedBaseSpecies(): void {
    this.addSpecies(
      { diet: 'herbivore', size: 1.0, maxSpeed: 1.4, senseRadius: 150, maxAge: 1000, hue: 125 },
      24,
      { name: 'Wild Grazers', spread: true },
    );
    this.addSpecies(
      { diet: 'carnivore', size: 1.35, maxSpeed: 1.8, senseRadius: 190, maxAge: 900, hue: 4 },
      10,
  ...
  private buildInputs(cell: Cell): number[] {
    ...
    if (wantsMeat) {
      ...
      for (const other of this.cells) {
        ...
        if (other.genome.size >= cell.genome.size * this.predationSizeRatio) continue;
    ...
    for (const other of this.cells) {
      ...
      if (cell.genome.size >= other.genome.size * this.predationSizeRatio) continue;
      const dx = other.x - cell.x;
      const dy = other.y - cell.y;
      const d = Math.hypot(dx, dy);
      if (d < bestThreatD) {
  ...
  private handlePredation(): void {
    for (const predator of this.cells) {
      ...
      for (const prey of this.cells) {
        ...
        if (prey.genome.size >= predator.genome.size * this.predationSizeRatio) continue;
