/**
 * Procedural binomial-style names for auto-speciated lineages — deliberately
 * deterministic from the founding genome's own sequence (a hash of its
 * symbols), not drawn from the world's rng stream, so the same genome
 * always names the same way and naming a species has no side effect on
 * simulation determinism/replay.
 */
import { GeneSequence } from './genes.js';
...
function hashSequence(sequence: GeneSequence): number {
  let h = 2166136261;
...
export function generateSpeciesName(sequence: GeneSequence): string {
