...
import { deriveEnergyCapturePower, derivePredationPower, genomeFromSequence, Genome, randomGenome } from './sim/genome.js';
...
function seedStartingPopulation(): void {
...
if (!SOUP_ENABLED && restored.status !== 'loaded') seedStartingPopulation();
...
el<HTMLButtonElement>('btn-reset').addEventListener('click', async () => {
  ...
  if (!SOUP_ENABLED) seedStartingPopulation();
...
document.querySelectorAll<HTMLButtonElement>('#tab-rail .tab-btn').forEach((btn) => {
  btn.addEventListener('click', () => {
    ...
    if (activeTab === 'species') refreshSpeciesSummaries(true);
  });
});
...
// --- manual soup-spawning tool (Chemistry tab) --------------------------
// Player-directed placement of specific chemistry into the pool — the
// same category of deliberate override the Designer form above already
// is, not part of the pool's own emergent economy. See
// Origin.spawnManualParticle/spawnManualPeptide/spawnManualRna's own
// comments (origin.ts) for why none of these touch the vent's cap.
type SpawnKind = 'aa' | 'nt' | 'lipid' | 'energy' | 'peptide' | 'rna';
...
let spawnArmed: SpawnKind | null = null;

const spawnPolymerFields = el('spawn-polymer-fields');
const fSpawnLength = el<HTMLInputElement>('f-spawn-length');
const fSpawnForce = el<HTMLInputElement>('f-spawn-force');
const spawnForceLabel = el('spawn-force-label');
const btnSpawnArm = el<HTMLButtonElement>('btn-spawn-arm');

function currentSpawnKind(): SpawnKind {
  return (document.querySelector('input[name="spawn-kind"]:checked') as HTMLInputElement)?.value as SpawnKind;
}

function refreshSpawnFields(): void {
...
document
  .querySelectorAll<HTMLInputElement>('input[name="spawn-kind"]')
...
const speciesTreeCanvas = el<HTMLCanvasElement>('chart-species-tree');
const speciesTreeEmpty = el<HTMLParagraphElement>('species-tree-empty');
const speciesShowExtinct = el<HTMLInputElement>('species-show-extinct');
const speciesCamera = defaultTreeCamera();
let speciesPositions = new Map<number, TreeNodeScreenPos>();
let selectedLineageId: number | null = null;
...
let speciesSummaries: SpeciesSummary[] = [];
let lastSpeciesSampleTick = -1;

function refreshSpeciesSummaries(force: boolean): void {
  ...
  if (!force && (tick === lastSpeciesSampleTick || tick % world.statsSampleInterval !== 0)) return;
  ...
  if (openedSpeciesLineageId !== null) {
    ...
    if (shown) renderSpeciesModal(shown);
...
function speciesTreeNodes(): Map<number, SpeciesNodeData> {
  const byId = new Map<number, SpeciesSummary>();
  for (const s of speciesSummaries) byId.set(s.lineageId, s);

  const keep = new Set<number>();
  if (speciesShowExtinct.checked) {
    for (const s of speciesSummaries) keep.add(s.lineageId);
  } else {
    for (const s of speciesSummaries) {
      if (s.isExtinct) continue;
      let cur: number | null = s.lineageId;
  ...
  const nodes = new Map<number, SpeciesNodeData>();
...
function selectSpecies(lineageId: number): void {
...
function renderSpeciesModal(s: SpeciesSummary): void {
  ...
  drawRadarChart(
    ...
    CATALYSIS_CLASSES.map((cls) => ({ label: cls, value: s.avgClassPower[cls] ?? 0 })),
    s.hue,
  );
}
...
interface LiveDetailRefs {
  ...
  /** Not a per-frame ref like the rest — the one anchor the gene editor
   * re-renders in place (entering/cancelling edit mode, expanding a
   * different gene) so toggling the editor doesn't rebuild the whole
   * panel and reset its scroll. An *applied* edit is different: it
   * changes stat blocks, the protein list and the radar too, so that path
   * takes the full rebuild instead (see refreshSelectionUI's genome-
   * identity check). */
  geneSection: HTMLElement;
}
...
let lastRenderedGenome: Genome | null = null;

/** An in-progress, uncommitted edit of one *living* individual's gene
 * sequence. Module-level and keyed by individual id rather than held in
 * the detail panel's DOM, because that DOM is not durable:
 * renderLiveDetail()/renderHistoricalDetail() both end in
 * treeDetail.replaceChildren(), and refreshSelectionUI() runs every single
 * frame — so if the individual dies mid-edit the panel flips to the
 * historical view and every edit node vanishes with it.
 *
 * `sequence` is a deep copy (cloneGeneSequence) taken at edit-start and
 * owned solely by this object, which is what makes mutating it in place on
 * each chip click safe *and* keeps the live simulation completely
 * unaffected until Apply. It is also why no auto-pause is needed: ticks
 * passing while the player edits provably cannot touch this. */
interface GeneDraft {
  individualId: number;
  sequence: GeneSequence;
  /** gene.join('') per gene at edit-start — the only thing "this gene has
   * been changed" is measured against. */
  baseline: readonly string[];
  /** Which single gene is currently expanded into chips, if any. Exactly
   * one at a time: see renderGeneSection's comment on why. */
  expandedIndex: number | null;
  error: string | null;
}
let geneDraft: GeneDraft | null = null;
...
const handEditedIds = new Set<number>();
...
function discardDraftIfOrphaned(live: Virtunism | null): void {
  if (geneDraft !== null && (live === null || live.id !== geneDraft.individualId)) geneDraft = null;
}
...
function selectIndividual(id: number | null): void {
  ...
  if (geneDraft !== null && geneDraft.individualId !== id) geneDraft = null;
...
function describeGeneEdit(cell: Virtunism, draft: GeneDraft, i: number): string {
  if (i < CORE_GENE_COUNT) {
    const name = CORE_GENE_LABELS[i] as keyof ReturnType<typeof decodeCoreTraits>;
    const was = cell.genome[name];
    const now = decodeCoreTraits(draft.sequence)[name];
    const fmt = (v: number | string): string =>
      typeof v === 'number' ? (Math.abs(v) >= 10 ? v.toFixed(0) : v.toFixed(2)) : v;
...
function applyGeneEdit(container: HTMLElement, cell: Virtunism): void {
  const draft = geneDraft;
  if (draft === null || draft.individualId !== cell.id) return;
  ...
  if (treeNode) treeNode.hue = genome.hue;
...
function renderGeneSection(container: HTMLElement, cell: Virtunism): void {
  container.replaceChildren();
  const draft = geneDraft !== null && geneDraft.individualId === cell.id ? geneDraft : null;
  const rerender = (): void => renderGeneSection(container, cell);
  ...
  draft.sequence.genes.forEach((gene, i) => {
    ...
    if (gene.join('') !== draft.baseline[i]) row.classList.add('edited');
    ...
    if (!expanded) {
      ...
      gene.forEach((symbol, j) => {
        ...
        if (symbol !== draft.baseline[i][j]) chip.classList.add('changed');
...
function refreshSelectionUI(): void {
  ...
  if (signature !== lastDetailSignature || genomeChanged) {
    ...
    if (keepScroll) treeDetail.scrollTop = prevScroll;
...
const treeCssPoint = (e: PointerEvent | WheelEvent): { x: number; y: number } => {
  const rect = treeCanvas.getBoundingClientRect();
  return { x: e.clientX - rect.left, y: e.clientY - rect.top };
};
const treeSize = (): { w: number; h: number } => ({
...
treeCanvas.addEventListener('pointerdown', (e) => {
  ...
  if (treePointers.size === 1) treeDragMoved = 0;
...
const endTreePointer = (e: PointerEvent): void => {
  treePointers.delete(e.pointerId);
  if (treePointers.size < 2) treePinchDist = 0;
};
...
el<HTMLButtonElement>('tree-fit').addEventListener('click', () => {
...
// gratuitous.
const speciesPointers = new Map<number, { x: number; y: number }>();
let speciesPinchDist = 0;
let speciesDragMoved = 0;

const speciesCssPoint = (e: PointerEvent | WheelEvent): { x: number; y: number } => {
  const rect = speciesTreeCanvas.getBoundingClientRect();
  return { x: e.clientX - rect.left, y: e.clientY - rect.top };
};
const speciesSize = (): { w: number; h: number } => ({
...
speciesTreeCanvas.addEventListener('pointerdown', (e) => {
  ...
  if (speciesPointers.size === 1) speciesDragMoved = 0;
...
const endSpeciesPointer = (e: PointerEvent): void => {
  speciesPointers.delete(e.pointerId);
  if (speciesPointers.size < 2) speciesPinchDist = 0;
};
...
el<HTMLButtonElement>('species-fit').addEventListener('click', () => {
  const fresh = defaultTreeCamera();
...
function frame(): void {
  if (!paused) {
    ...
    for (let i = 0; i < speed; i++) {
      ...
      if (SOUP_ENABLED) updateStage0Retirement();
...
const saveImportText = el<HTMLTextAreaElement>('save-import-text');
...
function setSaveModalStatus(message: string, kind: 'good' | 'bad' | null): void {
...
el<HTMLButtonElement>('btn-save-data').addEventListener('click', openSaveModal);
el<HTMLButtonElement>('save-alert-export').addEventListener('click', openSaveModal);
el<HTMLButtonElement>('save-modal-close').addEventListener('click', () => { saveModal.hidden = true; });
saveModal.addEventListener('click', (e) => {
  if (e.target === saveModal) saveModal.hidden = true;
});

el<HTMLButtonElement>('save-copy').addEventListener('click', async () => {
...
el<HTMLButtonElement>('save-import').addEventListener('click', async () => {
  ...
  const ok = await confirmDialog(
    `Restore that run (tick ${outcome.world.tick}, ${outcome.world.cells.length} alive)? Your current dish will be replaced.`,
  );
  if (!ok) return;
...
function installWorld(nextOrigin: Origin, nextWorld: World): void {
  origin = nextOrigin;
  world = nextWorld;
  applyPopCap();
  renderer.fitToWorld(world);
...
document.addEventListener('visibilitychange', () => {
  if (document.hidden) attemptSave();
});
