...
/** The two shapes this module actually calls, declared narrowly rather than
 * as a copy of the platform contract — everything else the runtime offers is
 * none of this file's business. `use` resolves later than the first
 * synchronous run and yields null when the capability cannot run here. */
...
export type DownloadOutcome =
  /** Written to disk, or handed to the host's share sheet. */
  | { status: 'saved'; via: 'viewer' | 'anchor' }
  /** The viewer was asked and said no (or let the prompt lapse). Never a
   * reason to retry automatically. */
  | { status: 'declined' }
  /** A real failure, with something specific to tell the player. */
 ...
 * anchor. */
function inViewerContext(): boolean {
...
const viewerDownloads: Promise<ViewerDownloads | null> = (async () => {
  if (!inViewerContext()) return null;
  try {
    ...
    return ns && typeof (ns as ViewerDownloads).save === 'function' ? (ns as ViewerDownloads) : null;
  } catch {
    return null;
  }
})();
...
function downloadViaAnchor(filename: string, text: string): DownloadOutcome {
  try {
...
function describeSaveError(err: unknown): DownloadOutcome {
 ...
 */
export async function offerRunDownload(filename: string, text: string): Promise<DownloadOutcome> {
