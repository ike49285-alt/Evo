...
export class Renderer {
  ...
  draw(
    ...
    options: { showVision?: boolean; highlightId?: number | null; hidePool?: boolean } = {},
  ): void {
    const ctx = this.ctx;
    this.animT += 1;
    ctx.save();
    ...
    if (typeof world.lightAt === 'function' && world.lightCols > 0) {
      ...
      for (let ry = 0; ry < world.lightRows; ry++) {
        for (let rx = 0; rx < world.lightCols; rx++) {
          ...
          if (b.x < 0 || b.y < 0 || a.x > this.viewportWidth || a.y > this.viewportHeight) continue;
    ...
    if (!options.hidePool) this.drawPool(origin, poolOffset);
  ...
  private drawCell(cell: Virtunism, p: { x: number; y: number }, r: number, showVision: boolean): void {
    ...
    if (showVision) {
      for (const pr of cell.genome.proteins) {
        if (pr.fold.catalysisClass !== 'photoreceptor') continue;
    ...
    for (const pr of cell.genome.proteins) {
      if (pr.fold.catalysisClass !== 'motor') continue;
    ...
    for (const pr of cell.genome.proteins) {
      if (pr.fold.catalysisClass !== 'lipidsynthase') continue;
      const strength = pr.fold.catalysisStrength;
      const mountAngle = cell.heading + pr.angle;
    ...
    for (const pr of cell.genome.proteins) {
      if (pr.fold.catalysisClass !== 'peptidyl') continue;
      const strength = pr.fold.catalysisStrength;
      const mountAngle = cell.heading + pr.angle;
    ...
    // used to play.
    for (const pr of cell.genome.proteins) {
      if (pr.fold.catalysisClass !== 'protease') continue;
      const strength = pr.fold.catalysisStrength;
      const mountAngle = cell.heading + pr.angle;
